...
from ..models.estate_models import EstateSell, EstateDeal
...
def get_discounts_with_summary():
    """
    Получает данные для страницы "Система скидок" с детальным логированием расчета средней цены.
    """
    print("\n" + "=" * 50)
    print("[ЛОГ СКИДОК] 🏁 НАЧАЛО ФОРМИРОВАНИЯ ОТЧЕТА")
    active_version = g.company_db_session.query(planning_models.DiscountVersion).filter_by(is_active=True).first()
    if not active_version:
        print("[ЛОГ СКИДОК] ❌ Активная версия скидок не найдена. Отчет будет неполным.")
        return {}

    all_discounts = active_version.discounts
    comments_map = {c.complex_name: c.comment for c in active_version.complex_comments}

    # --- ИЗМЕНЕНИЕ: ИСПОЛЬЗУЕМ defaultdict для упрощения ---
    discounts_map = defaultdict(list)
    for d in all_discounts:
        discounts_map[d.complex_name].append(d)

    sold_statuses = current_user.company.sale_statuses
    sold_sell_ids_query = g.mysql_db_session.query(EstateDeal.estate_sell_id).filter(
        EstateDeal.deal_status_name.in_(sold_statuses)
    )
    sold_sell_ids = {row[0] for row in sold_sell_ids_query.all()}
    print(f"[ЛОГ СКИДОК] ✅ Найдено {len(sold_sell_ids)} проданных объектов со статусами: {sold_statuses}")

    all_sells = g.mysql_db_session.query(EstateSell).options(joinedload(EstateSell.house)).all()
    sells_by_complex = defaultdict(list)
    for s in all_sells:
        if s.house:
            sells_by_complex[s.house.complex_name].append(s)

    # --- НОВЫЙ БЛОК ЛОГИРОВАНИЯ ДЛЯ ДИАГНОСТИКИ ---
    discount_complex_names = set(discounts_map.keys())
    sells_complex_names = set(sells_by_complex.keys())

    unmatched_discounts = discount_complex_names - sells_complex_names
    if unmatched_discounts:
        print("\n[ЛОГ СКИДОК] ⚠️ ВНИМАНИЕ: Найдены ЖК в настройках скидок, для которых нет объектов в базе:")
        for name in sorted(list(unmatched_discounts)):
            print(f"  -> '{name}'")

    final_data = {}
    tag_fields = {'kd': 'КД', 'opt': 'ОПТ', 'gd': 'ГД', 'holding': 'Холдинг', 'shareholder': 'Акционер'}

    for complex_name in sorted(discounts_map.keys()):
        ...
        base_discount_100 = next((d for d in discounts_in_complex if
                                  d.property_type == planning_models.PropertyType.FLAT and d.payment_method == planning_models.PaymentMethod.FULL_PAYMENT),
                                 None)
        ...
        base_discount_mortgage = next((d for d in discounts_in_complex if
                                       d.property_type == planning_models.PropertyType.FLAT and d.payment_method == planning_models.PaymentMethod.MORTGAGE),
                                      None)
        ...
        total_discount_rate = sum(
            getattr(base_discount_100, f, 0) or 0 for f in ['mpp', 'rop', 'kd', 'action']) if base_discount_100 else 0

        remainder_prices_per_sqm = []
        sells_in_complex = sells_by_complex.get(complex_name, [])

        for sell in sells_in_complex:
            if sell.id in sold_sell_ids or not sell.estate_sell_category:
                continue

            try:
                prop_type_enum = planning_models.PropertyType[sell.estate_sell_category.upper()]
            except KeyError:
                continue

            if prop_type_enum == planning_models.PropertyType.FLAT and sell.estate_price and sell.estate_area and sell.estate_area > 0:
        ...
        if remainder_prices_per_sqm:
            usd_rate = currency_service.get_current_effective_rate() or 1
            avg_price_uzs = sum(remainder_prices_per_sqm) / len(remainder_prices_per_sqm)
            avg_price_per_sqm_usd = avg_price_uzs / usd_rate
        ...
        for discount in discounts_in_complex:
            ...
            for field, tag_name in tag_fields.items():
                if getattr(discount, field, 0) or 0 > 0:
...
def _generate_version_comparison_summary(old_version, new_version, comments_data=None):
    ...
    for key, new_d in new_discounts.items():
        ...
        for field in ['mpp', 'rop', 'kd', 'opt', 'gd', 'holding', 'shareholder', 'action']:
            ...
            if abs(old_val - new_val) > 1e-9:
                ...
                diffs.append(
                    f"<b>{field.upper()}</b> {verb} {abs(delta * 100):.1f}% (с {old_val * 100:.1f}% до {new_val * 100:.1f}%)")
        if diffs:
    ...
    return render_template_string("""...""", old_v=old_version, new_v=new_version,
                                  changes=changes)  # HTML template left as is for brevity


def create_blank_version(comment: str):
    """Создает новую, ПУСТУЮ запись о версии скидок БЕЗ КОММИТА."""
    latest_version = g.company_db_session.query(planning_models.DiscountVersion).order_by(
...
def clone_version_for_editing(active_version):
    ...
    draft_version = planning_models.DiscountVersion(version_number=new_version_number,
    ...
    for old_discount in active_version.discounts:
        new_discount = planning_models.Discount(version_id=draft_version.id, **{k: getattr(old_discount, k) for k in
                                                                                ['complex_name', 'property_type',
                                                                                 'payment_method', 'mpp', 'rop', 'kd',
                                                                                 'opt', 'gd', 'holding', 'shareholder',
                                                                                 'action', 'cadastre_date']})
        g.company_db_session.add(new_discount)
    for old_comment in active_version.complex_comments:
        new_comment = planning_models.ComplexComment(version_id=draft_version.id, complex_name=old_comment.complex_name,
...
def update_discounts_for_version(version_id: int, form_data: dict, changes_json: str):
    ...
    discounts_map = {(d.complex_name, d.property_type.value, d.payment_method.value): d for d in
                     target_version.discounts}
